...
class BindicatorsManager {
  ...
  update(bindications, callback) {
    ...
    strips.forEach((strip) => {
      stripCommands.set(strip, [ 0x00 ]); // TODO
    });
    ...
    stripCommands.forEach((stripCommand, strip) => {
      self.ble.write(Buffer.from(stripCommand, 'hex'), (err) => {
        if(err) {
...
function createStripCommand(strip, leds, bindicator) {
  ...
  return [ 0x01, firstLedOffset, lastLedOffset, intensity.r, intensity.g,
           intensity.b, 0x01, 0x00, 0x00 ];
}
